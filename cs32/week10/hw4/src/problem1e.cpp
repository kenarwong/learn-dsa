...
namespace Problem1eNamespace
{
  ...
  void test()
  {
    ...
    for (vector<int>::iterator p = v2.begin(); p != v2.end(); p++, k++)
    {
      ...
      {
        for (int i = 0; i < 5; i++)
          v2.push_back(MAGIC);          // <-- This is the issue
                                        // iterators aren't valid for vectors after they have been modified
      }
    }
    if (allValid && k == 10)
